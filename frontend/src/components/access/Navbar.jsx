  ...
  return (
    <nav className="fixed z-200 mb-24 top-5 left-1/2 -translate-x-1/2 z-50 w-full flex justify-center font-['Poppins']">
      <div
        ...
        "
      >
        <h1 className="text-xl font-bold text-white">Hamsaraj V.C</h1>
        ...
        <button
          ...
          className="md:hidden hover:cursor-pointer text-white text-3xl hover:text-purple-400 transition"
        >
          {isOpen ? "✕" : "☰"}
        </button>
      </div>
      ...
      {admin && (
        <form
          ...
          className="absolute left-1/2 top-80 w-80 -translate-x-1/2 -translate-y-1/2 rounded-xl border backdrop-blur-2xl border-gray-700 bg-[#111827]/80 p-6 shadow-xl"
        >
          <div className="flex justify-end">
            <button
              type="button"
              onClick={() => setAdmin(false)}
              className="text-lg text-gray-400 transition hover:cursor-pointer hover:text-white"
            >
              ✕
            </button>
          </div>
          ...
          <div className="mb-4 flex flex-col gap-2">
            {error && (
              <div className="w-full px-2 h-fit text-red-500 border border-red-700 rounded-xl">
                <p className="w-fit h-fit text-center">
                  Invalid Password or Pass key !
                </p>
              </div>
            )}
